...
const SingleTaskOnGoing = ({ refetch, refetchCompleted, userId, id, title, description, deadLine, priority }) => {
        ...
        publicAxios.delete(`/ongoing/${id}`)
            .then(res => {
                ...
                if (res.data.deletedCount >= 1) {
                    ...
                    publicAxios.post("/completed", todoData)
                        .then(res => {
                            if (res.data.acknowledged) {
                                Swal.fire({
                                    position: "center",
                                    icon: "success",
                                    title: `Task moved to completed`,
                                    showConfirmButton: false,
                                    timer: 2500
                                });
                                refetch();
    ...
    const onSubmit = data => {
        ...
        publicAxios.patch(`/ongoing/${id}`, data)
            .then((res) => {
                console.log(res);
                if (res.data.acknowledged) {
    ...
    return (
        <div className="border-4 p-4 hover:cursor-move bg-white">
            <div className="flex justify-end">
                <button onClick={openModal} className="btn text-white text-xl bg-green-600"><FaRegEdit /></button>
            ...
            <div className="mt-4">
                <button onClick={handleComplete} className=" btn uppercase hover:bg-green-800 w-full text-white bg-green-600">Complete</button>
            </div>
            ...
            >
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="w-full bg-white  p-4 md:p-10 font-opensans">
                    ...
                    <div className="form-control mt-6">
                        <button className="btn uppercase btn-primary hover:bg-orange-500 bg-orange-600 border-none text-white">Update Task</button>
                    </div>
...
export default SingleTaskOnGoing;